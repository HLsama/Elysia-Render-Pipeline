...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    internal class DeferredLights
    {
        internal static class ShaderConstants
        {
            ...
            //添加新的模板属性（爱莉）
            public static readonly int _EyEyeBlendDirStencilRef = Shader.PropertyToID("_EyEyeBlendDirStencilRef");
            public static readonly int _EyEyeBlendDirStencilReadMask = Shader.PropertyToID("_EyEyeBlendDirStencilReadMask");
            public static readonly int _EyEyeBlendDirStencilWriteMask = Shader.PropertyToID("_EyEyeBlendDirStencilWriteMask");
        ...
        internal GraphicsFormat GetGBufferFormat(int index)
        {
            if (index == GBufferAlbedoIndex) // sRGB albedo, materialFlags
                ...
                return GraphicsFormat.R16G16B16A16_SFloat; // normal normal normal packedSmoothness//更改了法线的精度（爱莉）
        ...
        void RenderStencilDirectionalLights(CommandBuffer cmd, ref RenderingData renderingData, NativeArray<VisibleLight> visibleLights, int mainLightIndex)
        {
            ...
            for (int soffset = m_stencilVisLightOffsets[(int)LightType.Directional]; soffset < m_stencilVisLights.Length; ++soffset)
            {
                ...
                cmd.DrawMesh(m_FullscreenMesh, Matrix4x4.identity, m_StencilDeferredMaterial, 0, m_StencilDeferredPasses[(int)StencilDeferredPasses.EyeBlend_Elysia]);//新增的绘制Pass
                isFirstLight = false;
            }
        ...
        void RenderStencilPointLights(CommandBuffer cmd, ref RenderingData renderingData, NativeArray<VisibleLight> visibleLights)
        {
            ...
            for (int soffset = m_stencilVisLightOffsets[(int)LightType.Point]; soffset < m_stencilVisLights.Length; ++soffset)
            {
                ...
                cmd.DrawMesh(m_FullscreenMesh, Matrix4x4.identity, m_StencilDeferredMaterial, 0, m_StencilDeferredPasses[(int)StencilDeferredPasses.EyeBlend_Elysia]);//新增的绘制Pass（爱莉）
            }
        ...
        void InitStencilDeferredMaterial()
        {
            ...
            //为将模板属性传入shader（爱莉）
            m_StencilDeferredMaterial.SetFloat(ShaderConstants._EyEyeBlendDirStencilRef, (float)StencilUsage.MaterialEyeBlend);
            m_StencilDeferredMaterial.SetFloat(ShaderConstants._EyEyeBlendDirStencilReadMask, (float)StencilUsage.MaterialMask);
            m_StencilDeferredMaterial.SetFloat(ShaderConstants._EyEyeBlendDirStencilWriteMask, 0.0f);
        }
