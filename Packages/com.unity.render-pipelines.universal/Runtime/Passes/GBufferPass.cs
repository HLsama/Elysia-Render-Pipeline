...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    internal class GBufferPass : ScriptableRenderPass
    {
        ...
        static ShaderTagId s_ShaderTagEyNPRStrokeGBuffer = new ShaderTagId("EyNPRStrokeGBuffer");//添加描边Pass（爱莉）
        static ShaderTagId s_ShaderTagEyNPRHairGBuffer = new ShaderTagId("EyNPRHairGBuffer");//添加头发Pass（爱莉）
        static ShaderTagId s_ShaderTagEyNPREyeBlendGBuffer = new ShaderTagId("EyNPREyeBlendGBuffer");//添加一个新的延迟渲染阶段Pass（爱莉）
        ...
        public GBufferPass(RenderPassEvent evt, RenderQueueRange renderQueueRange, LayerMask layerMask, StencilState stencilState, int stencilReference, DeferredLights deferredLights)
        {
            ...
            {
                s_ShaderTagValues = new ShaderTagId[6];
                ...
                s_ShaderTagValues[4] = s_ShaderTagEyNPREyeBlendGBuffer;//新加入用于混合眼睛的ShaderTag
                s_ShaderTagValues[5] = new ShaderTagId(); // Special catch all case for materials where UniversalMaterialType is not defined or the tag value doesn't match anything we know.
            }

            if (s_RenderStateBlocks == null)
            {
                s_RenderStateBlocks = new RenderStateBlock[6];
                ...
                s_RenderStateBlocks[4] = DeferredLights.OverwriteStencil(m_RenderStateBlock, (int)StencilUsage.MaterialMask, (int)StencilUsage.MaterialEyeBlend);//新增的Stencil写入
                //s_RenderStateBlocks[5] = s_RenderStateBlocks[0];//注释此行可以解锁Gbuffer阶段的模板测试（爱莉）
            }
        }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, m_ProfilingSampler))
            {
                ...
                //添加新的LightMode，使其可以写入描边Pass
                ref CameraData cameraData = ref renderingData.cameraData;
                ShaderTagId lightModeTag = s_ShaderTagUniversalGBuffer;
                ShaderTagId lightModeTag2 = s_ShaderTagEyNPRHairGBuffer;
                ShaderTagId lightModeTag3 = s_ShaderTagEyNPRStrokeGBuffer;//添加描边Pass（爱莉）
                ...
                m_PassData.drawingSettings = CreateDrawingSettings(lightModeTag2, ref renderingData, renderingData.cameraData.defaultOpaqueSortFlags);
                ExecutePass(context, m_PassData, ref renderingData);
                m_PassData.drawingSettings = CreateDrawingSettings(lightModeTag3, ref renderingData, renderingData.cameraData.defaultOpaqueSortFlags);
                ExecutePass(context, m_PassData, ref renderingData);//添加描边Pass（爱莉）
